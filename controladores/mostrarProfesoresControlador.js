...
const {obtenerProfesores} = require('../modelos/mostrarProfesoresModelo.js');
...
const mostrarProfesores = (req, res) =>{
	obtenerProfesores((err, resultados)=>{
		if(err){
			return res.status(500).json({error: 'Hubo un error al obtener los Profesores'});
		}
