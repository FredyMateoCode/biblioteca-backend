...
const {obtenerEstudiantes} = require('../modelos/mostrarEstudiantesModelo.js');
...
const mostrarEstudiantes = (req, res) =>{
	obtenerEstudiantes((err, resultados)=>{
		if(err){
			return res.status(500).json({error: 'Hubo un error al obtener los Estudiantes'});
		}
		...
		console.log('Estudiantes Obtenidos', resultados);
	});
};
